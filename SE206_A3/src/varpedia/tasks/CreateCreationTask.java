...
public class CreateCreationTask  extends Task<Void> {
	...
	protected Void call() throws Exception {
		...
		//line 2,3: Create the initial slide show by reading in the images, which were previously renamed and numbered in order in OrderImagesController. 
		//		  This output will be deleted after it is used.
		//line 4,5,6: Add the text to the previously generated slideshow. This is saved to the videoFileName location, and is used in the final creation. 
		//		  There is potential for customisation here with font and/or colours.
		//line 7: Removing the extra files. This is important in case the user makes multiple creations in one session, as leaving these files will 
		//			 cause duplicate images in the slideshow and risk breaching Flickr's 30 image maximum user condition.
		//line 8: Combine the audio file and the video file to make the final creation video.
		String command = "image_Duration="+frameRate+";"
				+ " ffmpeg -y -framerate $image_Duration -i  ./creation_files/temporary_files/image_files/img%02d.jpg -c:v libx264 -r 24 ./creation_files/quiz_files/quiz_images/"
				+ fileTerm + ".mp4  > /dev/null; wait;"
				+ " ffmpeg -y -i ./creation_files/quiz_files/quiz_images/" + fileTerm + ".mp4 -vf \"drawtext=fontfile=myfont.tff:fontsize=" + fontSize + 
				":fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2:text='"+ AssociationClass.getInstance().getSearchTerm() + "'\" "
				+ "-codec:a copy ./creation_files/temporary_files/video_files/output.mp4 > /dev/null; wait;"
